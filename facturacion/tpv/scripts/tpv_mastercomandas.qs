...
/** \D
Abre una comanda
No se podr� abir una comanda si su arqueo est� cerrado
@return Boolean, devuelve true si todo se ha ejecutado correctamente y false si hay alg�n error
*/
function oficial_abrirComanda_clicked()
{
        ...
        if (cursor.valueBuffer("editable") == true) {
                MessageBox.warning(util.translate("scripts", "La venta ya est� abierta"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
                return true;
        }
...
@return Boolean, devuelve true si todo se ha ejecutado correctamente y fasle si hay alg�n error
*/
...
/** \D
Abre una transacci�n y llama a la funci�n ImprimirTiqueComanda
*/
...
/** \D
Si el m�dulo de informes no est� cargado muestra un mensaje de aviso y si lo est� lanza el informe correspondiente
@param codComanda codigo de la comanda a imprimir
@return true si se imprime correctamente y false si ha alg�n error
*/
...
/*
        ...
        while (qryTicket.next()) {
                if (primerRegistro) {
                        ...
                        flfact_tpv.iface.imprimirDatos("Cami�n", 20);
                        flfact_tpv.iface.imprimirDatos("�nigo, Pl�yade, C�scara");
                        flfact_tpv.iface.impAlinearH(2);
...
/** \D
Abre el caj�n del punto de venta actual
*/
...
/** \D
Abre el caj�n portamonedas conectado a una impresora
@impresora Nombre de la impresora LPR donde est� conectado el caj�n
*/
...
function oficial_imprimirFactura_clicked()
{
        ...
        var idFactura= cursor.valueBuffer("idfactura");
        if (!idFactura) {
                var res= MessageBox.warning(util.translate("scripts", "La venta seleccionada todav�a no tiene una factura asociada. �Desea crearla ahora?"), MessageBox.Yes, MessageBox.No);
                ...
                try {
                        ...
                        if (!idFactura) {
                                throw util.translate("scripts", "La funci�n crearFactura ha fallado");
                        }

                        /// Evita que se llame a sincronizarConFacturaci�n otra vez
...
/** \D Activa o desactiva el filtro que muestra �nicamente las �ltimas ventas o las del puesto por defecto. El filtro mejora el rendimiento
\end */
...
function ivaIncluido_imprimirTiquePOS(codComanda:String, impresora:String, qryTicket:FLSqlQuery)
{
	...
	flfact_tpv.iface.imprimirDatos("   " + agente);
	flfact_tpv.iface.impNuevaLinea();	
	flfact_tpv.iface.imprimirDatos(":: SIENTE :: Deporte, Turismo y Tecnolog�a", 45);
	...
	flfact_tpv.iface.imprimirDatos(":: Ordenadores :: Reparamos tu equipo inform�tico", 45);	
	flfact_tpv.iface.impNuevaLinea();
	flfact_tpv.iface.impNuevaLinea();
	flfact_tpv.iface.impNuevaLinea();
	flfact_tpv.iface.impNuevaLinea();
